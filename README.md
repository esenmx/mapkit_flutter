...
> **iOS only.** Apple's MapKit does not exist on Android. Pair this with `google_maps_flutter` behind a platform switch for cross-platform apps.
...
Minimum: iOS 17, Dart 3.10, Flutter 3.41.
...
- `MapKitUnsupportedPlatformException` — `MKMapView` built off iOS.
...
The generated files (`lib/src/messages.g.dart`, `ios/Classes/messages.g.swift`) are committed and must not be hand-edited. Wire types keep a `Platform` prefix so the generated Swift never shadows real MapKit symbols; public Dart names are restored via `typedef` (e.g. `MKUserTrackingMode`).
